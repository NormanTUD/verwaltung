...
class TopologyTranslator:
    ...
    def topology_detector(self, data: list[Record]):
        ...
        if len(data) > TOPOLOGY_DATA_MAX_WARN:
            self.log.warning(
                f"Topology Translation may be to expansive with big data-responses. {len(data)=}"
            )
    ...
    def get_topology_tree(
        self, recursion_depth: int = SAME_TYPE_DEPTH
    ) -> list[TopologyTree]:
        ...
        if not roots:
            self.log.warning(
                "No root nodes found — possible full cycle. "
    ...
    def _build_tree(self, node: TopologyNode, ancestors: set) -> TopologyTree:
        ...
        same_type = [
            (child, rel)
            for child, rel in node.connected_to
        ...
        same_type_info = None
        if same_type:
            same_type_info = SameTypeLoopInfo(relations=[rel for _, rel in same_type])
    ...
    def extract_node_types_and_relations(
        self, data: list[Record]
    ) -> tuple[set[str], set[AbstractRelation]]:
    ...
    def _rec_tree_eval(self, node, max_depth, depth=0, ancestors=None):
        ...
        if node.node_lbl in ancestors:
            indent = "    " * depth
            log.debug(
                f"{indent}↩ [{node.node_lbl}] (CROSS_TYPE_LOOP — already in this path)"
            )
            return
        ...
        same_type = [
            (c, r) for c, r in node.connected_to if c.node_lbl == node.node_lbl
        ]
        cross_type = [
            (c, r) for c, r in node.connected_to if c.node_lbl != node.node_lbl
        ]
        ...
        if same_type:
            ...
            log.debug(
                f"{indent}[{node.node_lbl}] {node.get_classification()} "
