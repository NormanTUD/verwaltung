    ...
    for record in data:
        ...
        for element in record:
            ...
            if isinstance(element, Node):
                ...
                for prop, val in element.items():
                    cells.append(
                        {"nodeId": element.element_id, "nodeType": label, "value": val}
                    )
...
def topological_rec_to_json(data: list[Record], params: ReadRequest) -> Response:
    ...
    return jsonify({"columns": columns, "rows": rows, "topology_meta": topology_meta})
...
def parse_request_params(req) -> ReadRequest:
    ...
    rel_filter = (
        [r.strip() for r in relationships_raw.split(",")] if relationships_raw else None
    )
    ...
    return ReadRequest(
        selected_labels, limit, property_filters, rel_filter, rel_as_filters
    )
