...
SAMPLE_MAPPING = {
    "nodes": {"Person": ["name"], "Location": ["city", "country"]},
    "relationships": [{"from": "Person", "to": "Location", "type": "LIVES_IN"}],
}
...
class TestNeo4jApp(unittest.TestCase):
        ...
        try:
            for attempt in range(15):  # max 15 Versuche
                try:
                    ...
                    print(
                        f"[{attempt+1}/15] Neo4j nicht bereit, warte 2 Sekunden... ({e})"
                    )
                    time.sleep(2)
    ...
    def test_update_multiple_nodes(self):
        ...
        response = self.app.put(
            "/api/update_nodes",
            data=json.dumps(
                {"ids": node_ids, "property": "status", "value": "updated"}
            ),
        ...
        results = self.graph.run(
            f"MATCH (n) WHERE ID(n) IN {node_ids} RETURN n.status AS status"
        ).data()
        for res in results:
    ...
    def test_update_node_with_nonexistent_id(self):
        ...
        response = self.app.put(
            f"/api/update_node/{non_existent_id}",
    ...
    def test_update_nodes_with_invalid_data(self):
        ...
        response = self.app.put(
            "/api/update_nodes",
            data=json.dumps({"ids": [1], "property": "status"}),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_update_node_success(self):
        ...
        result = self.graph.run(
            "MATCH (n:Person) WHERE ID(n)=$id RETURN n.age AS age", id=node_id
        ).data()
    ...
    def test_delete_node_success(self):
        ...
        result = self.graph.run(
            "MATCH (n) WHERE ID(n)=$id RETURN n", id=person_id
        ).data()
    ...
    def test_save_mapping_with_nodes(self):
        ...
        response = self.app.post(
            "/save_mapping", data=json.dumps(mapping), content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_save_mapping_with_relationship(self):
        ...
        mapping = {
            "nodes": {
                ...
                "Ort": [{"original": "city", "renamed": "name"}],
            },
            "relationships": [{"from": "Person", "to": "Ort", "type": "WOHNT_IN"}],
        }
    ...
    def test_overview_empty_db(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            b"overview", response.data.lower()
    ...
    def test_add_column_success(self):
        ...
        result = self.graph.run(
            "MATCH (n:Person) RETURN n.status AS status, n.name AS name"
        ).data()
    ...
    def test_add_column_empty_json(self):
        ...
        response = self.app.post(
            "/api/add_column", data="", content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
        self.assertIn(b"Request-Body ist leer", response.data)

        response2 = self.app.post(
            "/api/add_column", data="INVALID_JSON", content_type="application/json"
        )
        self.assertEqual(response2.status_code, 400)
    ...
    def test_upload_large_csv(self):
        ...
        response = self.app.post(
            "/upload", data={"data": large_csv}, content_type="multipart/form-data"
        )
        self.assertEqual(response.status_code, 200)
        with self.app.session_transaction() as sess:
    ...
    def test_upload_csv_with_missing_and_extra_columns(self):
        ...
        response = self.app.post(
            "/upload", data={"data": csv_data}, content_type="multipart/form-data"
        )
        self.assertEqual(response.status_code, 200)
        with self.app.session_transaction() as sess:
            self.assertIn("headers", sess)
    ...
    def test_graph_data_with_cyclic_relationship(self):
        ...
        self.assertTrue(
            any(
                link["source"] == n1.identity and link["target"] == n2.identity
                for link in data["links"]
            )
        )
        self.assertTrue(
            any(
                link["source"] == n2.identity and link["target"] == n1.identity
                for link in data["links"]
            )
        )
    ...
    def test_get_data_as_table_empty_db_returns_empty(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,Ort,Stadt"}
            )
    ...
    @pytest.mark.xfail(reason="Marked as an Issue", strict=True)
    def test_get_data_as_table_basic_positive_single_person(self):
        ...
        with self.app as client:
            resp = client.get(
                ...
                query_string={
                    "nodes": "Person,Ort,Stadt",
                    "relationships": "wohnhaft,teil_von",
                },
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
            ...
            unique_rows = list(
                {frozenset(r.items()): r for r in matching_rows}.values()
            )
    ...
    @pytest.mark.skip("filterLabels is deprecated")
    def test_get_data_as_table_filter_labels_behavior(self):
        ...
        with self.app as client:
            ...
            resp_person_only = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person,Ort,Stadt", "filterLabels": "Person"},
            )
            ...
            self.assertTrue(
                all(c["nodeType"] == "Person" for c in data_person["columns"])
            )

            # filter to only Ort -> expect Ort columns (and rows may exist based on pivoting)
            resp_ort_only = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person,Ort,Stadt", "filterLabels": "Ort"},
            )
    ...
    def test_get_data_as_table_invalid_limit_param_two_nodes(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person,Ort", "limit": "nope"},
            )
            self.assertEqual(resp.status_code, 500)
    ...
    def test_get_data_as_table_person_stadt_full_table(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person,Stadt", "relationships": "wohnt"},
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
            ...
            self.assertEqual(
                sorted(actual_rows, key=safe_sort_key),
    ...
    def test_get_data_as_table_complex(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={
                    "nodes": "Person,Stadt,Company,Hobby",
                    "relationships": "wohnt, arbeitet_bei, mag",
                },
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
    ...
    def test_add_property_with_null_value(self):
        ...
        props = graph.run(
            "MATCH (p:Person {name:'Clara'}) RETURN p.nickname AS nickname"
        ).data()[0]
    ...
    def test_add_property_with_float_value(self):
        ...
        self.assertEqual(resp.status_code, 200)
        val = graph.run("MATCH (p:Person {name:'Eva'}) RETURN p.score AS score").data()[
            0
        ]["score"]
    ...
    def test_create_node_with_empty_connectTo(self):
        resp = self.app.post(
            "/api/create_node",
            data=json.dumps(
                {"property": "stadt", "value": "LeereVerbindungen", "connectTo": []}
            ),
            content_type="application/json",
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_create_node_none_body(self):
        resp = self.app.post(
            "/api/create_node", data=None, content_type="application/json"
        )
        self.assertEqual(resp.status_code, 400)
        self.assertIn("ungültig", resp.get_json()["message"].lower())
    ...
    def test_add_property_partial_update(self):
        ...
        self.assertEqual(data["updated"], 1)
        age_anna = graph.run(
            "MATCH (p:Person {name:'Anna'}) RETURN p.age AS age"
        ).data()[0]["age"]
        age_ben = graph.run("MATCH (p:Person {name:'Ben'}) RETURN p.age AS age").data()[
            0
        ]["age"]
    ...
    def test_update_node_with_none_value(self):
        ...
        self.assertEqual(response.status_code, 200)
        val = self.graph.run(
            "MATCH (n:Person) WHERE ID(n)=$id RETURN n.nickname AS nickname",
            id=node.identity,
        ).data()[0]["nickname"]
    ...
    def test_delete_node_with_multiple_relationships(self):
        ...
        self.graph.create(
            Relationship(n1, "HAT_WOHNSITZ", n2)
            | Relationship(n1, "HAT_ZWEITWOHNUNG", n3)
        )
    ...
    def test_update_node_with_string_value(self):
        ...
        self.graph.create(n)
        self.app.put(
            f"/api/update_node/{n.identity}",
            data=json.dumps({"property": "nickname", "value": "Tommy"}),
            content_type="application/json",
        )
        val = self.graph.run(
            "MATCH (n:Person) WHERE ID(n)=$id RETURN n.nickname AS nickname",
    ...
    def test_update_node_with_boolean_value(self):
        ...
        self.app.put(
            f"/api/update_node/{n.identity}",
            data=json.dumps({"property": "flag", "value": True}),
            content_type="application/json",
        )
        val = self.graph.run(
            "MATCH (n:Person) WHERE ID(n)=$id RETURN n.flag AS flag", id=n.identity
        ).data()[0]["flag"]
        self.assertTrue(val)
    ...
    def test_update_node_with_float_value(self):
        ...
        self.app.put(
            f"/api/update_node/{n.identity}",
            data=json.dumps({"property": "score", "value": 3.14}),
            content_type="application/json",
        )
        val = self.graph.run(
            "MATCH (n:Person) WHERE ID(n)=$id RETURN n.score AS score", id=n.identity
        ).data()[0]["score"]
        self.assertEqual(val, 3.14)
    ...
    def test_delete_node_with_relations_to_multiple_nodes(self):
        ...
        self.graph.create(n1 | n2 | n3)
        self.graph.create(
            Relationship(n1, "VISITS", n2) | Relationship(n1, "VISITS", n3)
        )
        response = self.app.delete(f"/api/delete_node/{n1.identity}")
        self.assertEqual(response.status_code, 200)
        remaining = self.graph.run("MATCH (n) RETURN n").data()
    ...
    def test_delete_node_with_cascade_relations(self):
        ...
        rel_count = self.graph.run("MATCH ()-[r]->() RETURN COUNT(r) AS c").data()[0][
            "c"
        ]
    ...
    def test_update_node_multiple_properties(self):
        ...
        self.app.put(
            f"/api/update_node/{n.identity}",
            data=json.dumps({"property": "age", "value": 30}),
            content_type="application/json",
        )
        self.app.put(
            f"/api/update_node/{n.identity}",
            data=json.dumps({"property": "score", "value": 9.5}),
            content_type="application/json",
        )
        data = self.graph.run(
            "MATCH (n:Person) WHERE ID(n)=$id RETURN n.age AS age, n.score AS score",
            id=n.identity,
        ).data()[0]
    ...
    def test_get_data_as_table_only_main_nodes_no_edges(self):
        ...
        with self.app as client:
            ...
            values = {
                c["property"]: data["rows"][0]["cells"][i]["value"]
    ...
    def test_get_data_as_table_multiple_labels_on_node(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,VIP"}
            )
            data = resp.get_json()
    ...
    def test_get_data_as_table_multiple_paths_same_main(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,Ort"}
            )
            data = resp.get_json()
            cols = {(c["nodeType"], c["property"]) for c in data["columns"]}
            self.assertIn(("Ort", "plz"), cols)

    @pytest.mark.skip(reason="Why Would we do this?")
    ...
    def test_get_data_as_table_min_dist_fallback(self):
        ...
        with self.app as client:
            ...
            data = resp.get_json()
            vals = [
                c["value"] for row in data["rows"] for c in row["cells"] if c["value"]
            ]
    ...
    def test_get_data_as_table_returns_relations(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person,Ort", "relationships": "REL"},
            )
    ...
    def test_get_data_as_table_mixed_labels_in_query(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,Ort,Alien"}
            )
            data = resp.get_json()
            self.assertEqual(len(data["rows"]), 1)
    ...
    def test_get_data_as_table_relations_between_mains(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person", "relationships": "KNOWS"}
            )
            rels = resp.get_json()["rows"][0]["relations"]
    ...
    def test_get_data_as_table_respects_maxdepth_one(self):
        ...
        self.graph.run("MATCH (n) DETACH DELETE n")
        self.graph.run(
            "CREATE (p:Person {name:'P'})-[:R]->(o:Ort {plz:'11'})-[:R]->(s:Stadt {stadt:'S'})"
        )
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person,Ort,Stadt", "maxDepth": "1"},
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
    ...
    def test_get_data_as_table_relation_types_captured(self):
        ...
        with self.app as client:
            ...
            for _ in range(5):  # max 5 Versuche
                resp = client.get(
                    "/api/get_data_as_table", query_string={"nodes": "Person,Ort", "relationships": "FOO"}
                )
                self.assertEqual(resp.status_code, 200)
                data = resp.get_json()
                rows = data.get("rows", [])
            ...
            # 4. Assertion
            self.assertTrue(
                rel_found, "FOO-Relation wurde nicht in der API-Ausgabe gefunden"
            )
    ...
    def test_get_data_as_table_duplicate_relationships_deduped(self):
        ...
        self.graph.run(
            "MATCH (a:Person {name:'A'}), (b:Ort {plz:'101'}) CREATE (a)-[:REL]->(b)"
        )
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,Ort"}
    ...
    def test_get_data_as_table_nodes_with_multiple_labels_and_filter(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person,VIP,Ort", "filterLabels": "VIP"},
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
            cols = {(c["nodeType"], c["property"]) for c in data["columns"]}
    ...
    def test_get_data_as_table_limit_reduces_rows(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person", "limit": "3"}
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
    ...
    def test_get_data_as_table_label_with_spaces_in_param(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": " Ort , Stadt "}
            )
            self.assertEqual(resp.status_code, 200)
    ...
    def test_get_data_as_table_unicode_label_handling(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Städte"}
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
    ...
    def test_get_data_as_table_relations_unique_when_created_twice(self):
        ...
        self.graph.run(
            "MATCH (p:Person {n:'x'}), (o:Ort {plz:'9'}) CREATE (p)-[:R]->(o)"
        )
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,Ort"}
    ...
    def test_get_data_as_table_high_maxdepth_no_crash(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person,Ort", "maxDepth": "10"},
            )
    ...
    def test_get_data_as_table_nonexistent_filterlabel_no_crash(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person", "filterLabels": "Alien"},
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
    ...
    def test_get_data_as_table_relations_between_mains_reported(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person", "relationships": "knows"},
            )
            ...
            found = any(
                any(r["relation"] == "knows" for r in row["relations"])
                for row in data["rows"]
            )
    ...
    def test_get_data_as_table_graph_with_large_number_of_edges(self):
        ...
        for i in range(20):
            self.graph.run(
                "MATCH (p:Person {n:1}) CREATE (o:Ort {plz:toString(%d)}) CREATE (p)-[:R]->(o)"
                % i
            )
        with self.app as client:
            resp = client.get(
    ...
    def test_get_data_as_table_query_with_many_label_names(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person,Ort,Stadt,Alien,VIP"},
            )
            self.assertEqual(resp.status_code, 200)
    ...
    def test_get_data_as_table_cyclic_graph_with_depth_limit(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person", "maxDepth": "5"},
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
    ...
    def test_get_data_as_table_property_edge_cases(self):
        ...
        with self.app as client:
            ...
            self.assertTrue(
                any(
                    cell["value"] in ("", None, 0, False, long_str)
                    for row in data["rows"]
                    for cell in row["cells"]
                )
            )
    ...
    def test_get_data_as_table_array_and_map_properties(self):
        ...
        self.graph.run(
            "CREATE (:Person {tags:$tags, meta:$meta})", tags=["a", "b"], meta=meta_str
        )
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person"}
    ...
    def test_get_data_as_table_parallel_relationships(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,Stadt", "relationships": "R1"}
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
            self.assertEqual(len(data["rows"]), 1)
    ...
    def test_get_data_as_table_self_loop_relationship(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person", "maxDepth": "2"},
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
    ...
    def test_get_data_as_table_large_graph_with_limit(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person", "limit": "10"},
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
    ...
    def test_get_data_as_table_multiple_labels_and_relations(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person,Employee,Department,Team,City,Place"},
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
            self.assertGreaterEqual(len(data["rows"]), 1)
    ...
    def test_get_data_as_table_cyclic_graph(self):
        ...
        self.graph.run("MATCH (n) DETACH DELETE n")
        self.graph.run("CREATE (n:Node {name:'Loop'}) " "CREATE (n)-[:CONNECTS]->(n)")
        with self.app as client:
    ...
    def test_get_data_as_table_deep_hierarchy(self):
        ...
        tx.create(l)
        tx.create(k)
        tx.create(Relationship(p, "wohnt", o))
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={
                    "nodes": "Person", "relationships": ["wohnt", "liegt_in"],
                    "maxDepth": 5,
                }
            )
            self.assertEqual(resp.status_code, 200)
    ...
    def test_get_data_as_table_long_strings_and_unicode(self):
        ...
        self.graph.run("MATCH (n) DETACH DELETE n")
        long_bio = "😀" + "x" * 5000
        self.graph.run("CREATE (p:Person {vorname:'E', bio:$bio})", bio=long_bio)
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person"}
            )
            ...
            self.assertIn(
                long_bio,
                [cell["value"] for row in data["rows"] for cell in row["cells"]],
            )
    ...
    def test_get_data_as_table_nodes_with_multiple_adjacent_relations(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person", "relationships": "wohnt"}
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
    ...
    def test_get_data_as_table_optional_nodes_and_missing_paths(self):
        ...
        with self.app as client:
            ...
            city_values = [
                cell["value"]
                for row in data["rows"]
                for col, cell in zip(data["columns"], row["cells"])
                if col["nodeType"] == "Stadt"
            ]
    ...
    def test_save_mapping_spaces_in_field_names(self):
        ...
        csv_data = "full name\nAlice"
        mapping = {
            "nodes": {"Person": [{"original": "full name", "renamed": "name"}]},
            "relationships": [],
        }
    ...
    def test_create_node_basic_success(self):
        ...
        with patch("app.graph.run") as mock_run:
            ...
            resp = self.app.post(
                "/api/create_node", json={"property": "name", "value": "Alice"}
            )
            self.assertEqual(resp.status_code, 200)
    ...
    def test_create_node_invalid_property_name(self):
        ...
        resp = self.app.post(
            "/api/create_node", json={"property": "123abc", "value": "Alice"}
        )
        self.assertEqual(resp.status_code, 400)
    ...
    def test_create_node_value_numeric_boolean(self):
        ...
        with patch("app.graph.run") as mock_run:
            ...
            resp1 = self.app.post(
                "/api/create_node", json={"property": "age", "value": 42}
            )
            resp2 = self.app.post(
                "/api/create_node", json={"property": "active", "value": True}
            )
    ...
    def test_create_node_missing_target_label_default_node(self):
        ...
        with patch("app.graph.run") as mock_run:
            ...
            resp = self.app.post(
                "/api/create_node", json={"property": "title", "value": "CEO"}
            )
            self.assertEqual(resp.status_code, 200)
    ...
    def test_get_data_as_table_person_ort_buch_stable(self):
        ...
        for vorname, nachname, plz, strasse, buch, jahr in persons:
            person_nodes.append(
                Node("Person", vorname=vorname, nachname=nachname, uid=uid)
            )
            ort_nodes.append(Node("Ort", plz=plz, straße=strasse, uid=uid))
            if buch:
                buch_nodes.append(
                    Node("Buch", titel=buch, erscheinungsjahr=jahr, uid=uid)
                )
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,Ort,Buch"}
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
        ...
        self.assertEqual(
            persons_found, expected_persons, f"Nur gefunden: {persons_found}"
        )
    ...
    def _wait_for_nodes(self, label, uid, expected_count, retries=10, delay=0.1):
        ...
        for _ in range(retries):
            count = self.graph.evaluate(
                f"MATCH (n:`{label}` {{uid:$uid}}) RETURN count(n)", uid=uid
            )
            if count == expected_count:
                return
            time.sleep(delay)
        raise RuntimeError(
            f"Nicht alle Nodes {label} mit uid={uid} gefunden. Erwartet: {expected_count}, gefunden: {count}"
        )
    ...
    def test_get_data_as_table_no_missing_nodes(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "NT2,dTyp2,Person,Ort"},
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
    ...
    def test_complex_multi_level_paths(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,Ort,Buch,NT2"}
            )
            data = resp.get_json()
            self.assertEqual(resp.status_code, 200)
    ...
    def test_nodes_with_multiple_labels(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,Buch"}
            )
            data = resp.get_json()
            values = [c["value"] for row in data["rows"] for c in row["cells"]]
    ...
    def test_nested_loops(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,Buch,NT2"}
            )
            data = resp.get_json()
            values = [c["value"] for row in data["rows"] for c in row["cells"]]
    ...
    def test_missing_properties_and_mixed_types(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Person,Buch,Ort"}
            )
            data = resp.get_json()
    ...
    def test_reset_and_load_data(self):
        ...
        # Check persons
        persons = self.graph.run(
            "MATCH (p:Person) RETURN p.vorname AS vorname, p.nachname AS nachname"
        ).data()
        ...
        # Check books
        books = self.graph.run(
            "MATCH (b:Buch) RETURN b.titel AS titel, b.erscheinungsjahr AS jahr"
        ).data()
        ...
        rels = self.graph.run(
            "MATCH (p:Person)-[:WOHNT_IN]->(o:Ort) RETURN p,o LIMIT 1"
        ).data()
    ...
    def test_duplicate_query_name(self):
        self.app.post(
            "/api/save_query", json={"name": "Duplicate", "selectedLabels": ["X"]}
        )
        resp = self.app.post(
            "/api/save_query", json={"name": "Duplicate", "selectedLabels": ["X"]}
        )
    ...
    def test_create_node_success(self):
        resp = self.app.post(
            "/api/create_node",
            json={"node_label": "Person", "props": {"name": "Alice"}},
        )
        self.assertEqual(resp.status_code, 200)
        data = resp.get_json()
        self.assertEqual(data["status"], "success")
    ...
    def test_create_node_invalid_props(self):
        resp = self.app.post(
            "/api/create_node", json={"node_label": "Person", "props": {}}
        )
        self.assertEqual(resp.status_code, 400)
        data = resp.get_json()
    ...
    def test_create_node_with_relationship(self):
        # zuerst ein Zielnode
        existing = self.graph.run(
            "CREATE (p:Person {name:'Bob'}) RETURN ID(p) AS id"
        ).data()[0]["id"]
    ...
    def test_add_row_filters_invalid_properties(self):
        ...
        data = {
            "label": "TestNode2",
            "properties": {"123invalid": "val", "validName": "ok"},
        }
        response = self.app.post("/api/add_row", json=data)
        self.assertEqual(response.status_code, 200)
    ...
    def _wait_for_node(self, label, uid, retries=10, delay=0.1):
        ...
        for _ in range(retries):
            node_id = self.graph.evaluate(
                f"MATCH (n:`{label}` {{uid:$uid}}) RETURN id(n)", uid=uid
            )
    ...
    def test_add_relationship_flakeless(self):
        ...
        with self.app as client:
            ...
            self.assertEqual(
                resp.status_code, 200, f"API-Fehler: {resp.get_data(as_text=True)}"
            )
    ...
    def test_add_relationship_invalid_property_names(self):
        ...
        self.assertIsNotNone(
            alice_id, f"Alice Node-ID konnte nicht ermittelt werden. Node: {alice}"
        )
        self.assertIsNotNone(
            berlin_id, f"Berlin Node-ID konnte nicht ermittelt werden. Node: {berlin}"
        )
    ...
    def _run_querybuilder_request(self, client, operator, field, value=None):
        ...
        if "rows" not in data:
            self.fail(
                f"Operator {operator} not implemented or invalid response: {data}"
            )
    ...
    def test_operator_not_equal(self):
        ...
        with self.app as client:
            _, data = self._run_querybuilder_request(
                client, "not_equal", "Buch.erscheinungsjahr", "1808"
            )
    ...
    def test_operator_not_in(self):
        ...
        with self.app as client:
            _, data = self._run_querybuilder_request(
                client, "not_in", "Buch.erscheinungsjahr", ["1808"]
            )
            values = [c.get("value") for r in data["rows"] for c in r["cells"]]
            self.assertNotIn("1808", values)
    ...
    def test_operator_not_contains(self):
        ...
        with self.app as client:
            _, data = self._run_querybuilder_request(
                client, "not_contains", "Buch.titel", "Process"
            )
            titles = [c.get("value") for r in data["rows"] for c in r["cells"]]
    ...
    def test_operator_not_ends_with(self):
        ...
        with self.app as client:
            _, data = self._run_querybuilder_request(
                client, "not_ends_with", "Buch.titel", "Liebe"
            )
            titles = [c.get("value") for r in data["rows"] for c in r["cells"]]
    ...
    def test_reset_and_load_complex_data(self):
        ...
        persons = self.graph.run(
            "MATCH (p:Person) RETURN p.vorname AS vorname, p.nachname AS nachname LIMIT 1"
        ).data()
    ...
    def test_get_data_as_table_multiple_orders_for_one_customer(self):
        ...
        with self.app as client:
            ...
            bestellnrs = [
                ...
                for row in rows
                for cell in row["cells"]
                if cell.get("nodeType") == "Bestellung"
            ]
    ...
    def test_get_data_as_table_with_cycle_customer_mayor(self):
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table", query_string={"nodes": "Kunde,Person,Stadt"}
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
    ...
    def test_get_data_as_table_orders_customers(self):
        ...
        for e in entries:
            ...
            """,
                vn=e["person"]["vorname"],
                nn=e["person"]["nachname"],
                titel=e["person"]["titel"],
                jahr=e["person"]["geburtsjahr"],
                email=e["kunde"]["email"],
                knr=e["kunde"]["kundennummer"],
                produkt=e["kunde"]["produkt"],
                bnr=e["bestellung"]["bestellnr"],
                betrag=e["bestellung"]["betrag"],
                datum=e["bestellung"]["datum"],
                status=e["bestellung"]["status"],
                tracking=e["lieferung"]["tracking"],
                versandart=e["lieferung"]["versandart"],
                versandnr=e["lieferung"]["versandnr"],
                stadt=e["stadt"]["stadt"],
            )
        ...
        with self.app as client:
            resp = client.get(
                "/api/get_data_as_table",
                query_string={"nodes": "Person,Kunde,Bestellung,Lieferung,Stadt"},
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.get_json()
